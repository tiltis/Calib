"""
Wave Orthorectification v10

v9 → v10 변경사항:
  - build_remap_tables: 세로(y)만 보정하던 것을 XY 동시 보정으로 개선
      · 기존(v9): map_x = col 그대로 → 가로 방향 원근 왜곡 미보정
                  → 결과 영상에서 파도가 여전히 방사형으로 보임
      · 신규(v10): 각 행 r에서 m/pixel(src_y[r]) / base_mpp = scale_x
                   map_x[r, col] = cx + (col - cx) / scale_x
                   → 먼 행(상단)의 좁은 파도 마루를 물리적 너비로 펼침
  - 파일 위치: 2026-04-06/piza3.py (piza2.py 수정 금지 방침)
"""
...
def build_remap_tables(roi_w, roi_h, mpp_map):
    """
    등거리 remap 테이블 생성 — XY 동시 보정 (v10)

    세로(Y):
      - 누적 거리 기준으로 src_y 보간 → 각 출력 행이 동일 물리 거리 간격

    가로(X):
      - 행 r의 source y = src_y[r]에서의 m/pixel = mpp_map[src_y[r]]
      - scale_x = mpp_map[src_y[r]] / base_mpp
      - map_x[r, col] = cx + (col - cx) / scale_x
      - 상단(먼 곳, scale_x > 1): 소스의 중앙 부근을 넓게 펼침 → 파도 마루 폭 복원
      - 하단(base row, scale_x = 1): x 변화 없음
    """
    ...
    cx    = roi_w / 2.0
    cols  = np.arange(roi_w, dtype=np.float32) - cx          # (roi_w,) 중앙 기준
    # np.outer(1/scale_x, cols) → (out_h, roi_w)
